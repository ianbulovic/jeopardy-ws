...
window.addEventListener("DOMContentLoaded", () => {
    ...
    var updateScoreButtons = () => {
        for (let i = 0; i < scoreButtons.length; i++) {
            ...
            const delta = (positiveScoreButtons ? 1 : -1) * (i + 1) * (doubleJeopardy ? 400 : 200);
            scoreButtons[i].textContent = `${delta}`
            scoreButtons[i].onclick = (event) => {
                websocket.send(JSON.stringify({action: "changeScore", target: selectedPlayer, delta: delta}));
            };
        }
        // same thing for custom value
        const customDelta = (positiveScoreButtons ? 1 : -1) * customScoreButtonValue;
        customScoreButton.textContent = `${customDelta}`;
        customScoreButton.onclick = (event) => {
            websocket.send(JSON.stringify({action: "changeScore", target: selectedPlayer, delta: customDelta}));
        };
    };
    updateScoreButtons();
    ...
    requestBetButton.onclick = (event) => {
        websocket.send(JSON.stringify({action: "requestBet", target: selectedPlayer}));
    };
    
    kickButton.onclick = (event) => {
        websocket.send(JSON.stringify({action: "kickPlayer", target: selectedPlayer}));
